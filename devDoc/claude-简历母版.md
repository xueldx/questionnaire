...
> 基于 React + NestJS 的全流程智能问卷平台，支持 AI 润色提示词、AI生成问卷与修改问卷与分析、拖拽式可视化编辑器、12 种物料组件体系，采用 pnpm Monorepo 管理 PC 端（Vite）、移动端（Next.js）、服务端（NestJS）三个子包。
...
面试时不要逐条罗列功能，而是讲一条故事线：用户输入提示词→让 AI润色提示词→ 生成一份问卷 →借助AI修改问卷→ 在编辑器里拖拽调整 → 发布给用户在移动端填写 → 最后用 AI 分析结果。每个环节讲一个你解决的技术难题。
